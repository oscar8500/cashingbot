...
class Config {
    constructor(path) {
        ...
        if (this.storage.envKey) {
            this.storageUrl = process.env[this.storage.envKey];
        }
        if (this.redis.uri) {
            this.redisUrl = this.redis.uri;
        }
        if (this.redis.envKey) {
            this.redisUrl = process.env[this.redis.envKey];
        }
        if (!this.seedMnemonic) {
            this.seedMnemonic = process.env['TOKEN_APP_SEED'];
        }
